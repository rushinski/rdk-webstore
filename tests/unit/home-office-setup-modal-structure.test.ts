...
describe("home office setup modal structure", () => {
  it("delegates the main address editor and relocation impact flow to focused components", () => {
    ...
    expect(source).toContain("@/components/admin/nexus/HomeOfficeAddressForm");
    expect(source).toContain("@/components/admin/nexus/HomeOfficeChangeImpactModal");
  ...
  it("delegates shared home office types to a focused module", () => {
    ...
    const typesSource = fs.readFileSync(
      path.join(process.cwd(), "src/components/admin/nexus/homeOfficeSetupTypes.ts"),
      "utf8",
    );
