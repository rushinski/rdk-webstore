...
export default function TransferSettingsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const weeklyAnchor = WEEKLY_ANCHORS.find(a => a.value === data.payout_schedule?.weekly_anchor)?.value ?? 'monday';
  ...
  const primaryBank = bankAccounts.find(b => b.default_for_currency) ?? bankAccounts[0] ?? null;
      ...
      <section className="rounded-sm bg-zinc-900 border border-zinc-800/70 p-6">
        ...
        {primaryBank ? (
          <div className="rounded-sm bg-zinc-950 border border-zinc-800/70 p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <a
                ...
                className="px-4 py-2 border border-zinc-800/70 text-sm text-zinc-300 hover:border-zinc-700 whitespace-nowrap"
              >
                Manage in Stripe
              </a>
            </div>
          ...
          <div className="rounded-sm bg-zinc-950 border border-zinc-800/70 p-6 text-center">
            <p className="text-zinc-400 text-sm mb-3">No bank account connected yet.</p>
            <a
              href="https://dashboard.stripe.com"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block px-4 py-2 bg-red-600 text-white text-sm hover:bg-red-500"
            >
        ...
        <div className="mt-4 p-4 rounded-sm bg-zinc-950 border border-zinc-800/70">
          <p className="text-xs text-zinc-500">
            <strong className="text-zinc-400">Note:</strong> Bank account management is handled securely through Stripe's dashboard. 
            You can add, remove, or change your default payout account there.
          </p>
        </div>
      </section>
